...
# 一、简介和概述  
在不依赖Qt的前提下，本GUI框架提供了类似Qt的API接口，使用c++，仅需要几百KB的硬件资源，适用于嵌入式软件。这是最容易理解的GUI框架，史上最简单，没有之一。   
...
## 3-2 授权许可协议（适用于普通参与者）  
 **本协议适用于未被列入《exception/black list》的普通参与者，被列入《exception/black list》的个人和实体不适用本协议。**  
