...
void GMImage::paintEvent ( GPainter& p )
{
    ...
    int nX = x(), nY = y();
    if( width() > imgFhl->m_imageScaled.width()+1 )
    {
        nX += (width()-imgFhl->m_imageScaled.width() )/2;
    }
    if( height() > imgFhl->m_imageScaled.height()+1 )
    {
        nY += (height()-imgFhl->m_imageScaled.height() )/2;
    }
